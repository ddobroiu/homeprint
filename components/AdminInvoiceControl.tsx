    ...
    return (
      <div className="flex gap-1">
        <Button 
          variant="outline" 
          size="sm" 
          className="h-7 w-full bg-amber-500/10 border-amber-500/30 text-amber-400 hover:bg-amber-500/20 text-[10px]"
        >
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent className="bg-[#09090b] border-white/10 text-white">
             ...
             <div className="py-4">
                ...
                {uploading && <p className="text-xs text-amber-400 mt-2 animate-pulse">Se încarcă...</p>}
             </div>
          </DialogContent>
        </Dialog>
      </div>
        ...
        <label 
          ...
          className={`flex items-center justify-center w-full h-7 rounded border text-[10px] font-medium cursor-pointer transition-all
            ${uploading 
              ...
              : 'bg-amber-600/10 border-amber-600/30 text-amber-400 hover:bg-amber-600 hover:text-white hover:border-amber-500'
            }`}
        >

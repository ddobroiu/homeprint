            ...
            {siteConfig.headerNav.map((item) =>
                item.highlight ? (
                    <Link
                        key={item.href}
                        href={item.href}
                        className="px-5 py-1.5 text-xs font-bold text-white bg-amber-700 rounded-md hover:bg-amber-800 transition-all tracking-wide uppercase"
                    >
                        {item.label}
                    </Link>
                ) : item.children ? (
                    ...
                    >
                        <button
                            ...
                            className="flex items-center gap-1.5 text-[13px] font-semibold text-stone-600 hover:text-amber-800 transition-colors py-4 cursor-pointer border-b-2 border-transparent hover:border-amber-700"
                        >
                            {item.label}
                            <ChevronDown
                                size={13}
                        ...
                        <div
                            className={`absolute top-full left-0 z-[1001] pt-2 w-max min-w-[240px] transition-all duration-200 ${
                                openDropdown === item.label
                                    ? "opacity-100 translate-y-0 pointer-events-auto visible"
                                    : "opacity-0 -translate-y-1 pointer-events-none invisible"
                            }`}
                        >
                            <div className="bg-[#FBF7F1] border border-stone-200 rounded-lg shadow-[0_22px_60px_-30px_rgba(28,25,23,0.35)] p-1.5">
                                {item.children.map((child) => (
                                    <Link
                                        key={child.href}
                                        href={child.href}
                                        onClick={() => setOpenDropdown(null)}
                                        className="block text-sm text-stone-700 hover:text-amber-800 hover:bg-white p-2.5 rounded-md transition-all font-semibold"
                                    >
                                        {child.label}
                                    </Link>
                                ))}
                    ...
                    <Link
                        key={item.href}
                        href={item.href}
                        className="text-[13px] font-semibold text-stone-600 hover:text-amber-800 transition-colors tracking-tight py-4 border-b-2 border-transparent hover:border-amber-700"
                    >
                        {item.label}
                    </Link>
                )
                ...
                <div className="container mx-auto px-4 w-full max-w-7xl h-16 sm:h-[80px] flex items-center justify-between gap-4">
                    ...
                    <div className="hidden lg:block flex-1 max-w-xl px-8">
                        <SearchBox
                            ...
                            className="w-full border border-stone-300 rounded-lg bg-white focus-within:ring-1 focus-within:ring-amber-600 focus-within:border-amber-600 transition-all"
                        />
                    </div>
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                        <Link
                            href="/editor"
                            className="hidden sm:flex items-center gap-2 px-4 py-2.5 bg-amber-700 hover:bg-amber-800 text-white rounded-lg font-semibold text-xs tracking-tight transition-all active:scale-[0.99] group"
                        >
                            <PencilRuler size={16} className="group-hover:rotate-12 transition-transform" />
                            Editor Online
                        </Link>

                        <div className="w-px h-6 bg-stone-300 hidden sm:block mx-1"></div>
                        ...
                        {session?.user ? (
                            <div className="relative account-dropdown hidden sm:block z-50">
                                <button
                                    ...
                                    className="flex items-center gap-2 p-1.5 pr-3 bg-white border border-stone-300 rounded-lg hover:border-amber-600 transition-colors"
                                >
                                    <div className="w-7 h-7 rounded-md bg-stone-900 flex items-center justify-center text-white font-bold text-xs">
                                        {(session.user.name?.[0] || session.user.email?.[0] || "U").toUpperCase()}
                                    </div>
                                    <span className="text-xs font-semibold text-stone-800 max-w-[80px] truncate">{session.user.name?.split(' ')[0] || 'Cont'}</span>
                                ...
                                <div className={`absolute top-[120%] right-0 w-64 transition-all duration-200 ease-out z-[999] origin-top-right ${isAccountOpen ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-95 pointer-events-none'}`}>
                                    <div className="bg-white rounded-xl shadow-[0_22px_60px_-30px_rgba(28,25,23,0.35)] border border-stone-200 overflow-hidden">
                                        <div className="px-4 py-3 bg-stone-50 border-b border-stone-100">
                                            <p className="text-sm font-bold text-stone-900 truncate">{session.user.name}</p>
                                            <p className="text-[11px] text-stone-500 font-semibold truncate uppercase tracking-wider mt-0.5">{session.user.email}</p>
                                        </div>
                                        <div className="p-2 space-y-0.5">
                                            <Link href="/account?tab=orders" onClick={() => setIsAccountOpen(false)} className="flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-stone-700 hover:bg-stone-50 hover:text-stone-950 rounded-lg transition-all">
                                                <Package size={16} /> Comenzile mele
                                            </Link>
                                            <Link href="/account?tab=favorites" onClick={() => setIsAccountOpen(false)} className="flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-stone-700 hover:bg-stone-50 hover:text-stone-950 rounded-lg transition-all">
                                                <Heart size={16} /> Favorite
                                            </Link>
                                            <Link href="/account" onClick={() => setIsAccountOpen(false)} className="flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-stone-700 hover:bg-stone-50 hover:text-stone-950 rounded-lg transition-all">
                                                <User size={16} /> Profilul meu
                                            </Link>
                                            <Link href="/account?tab=addresses" onClick={() => setIsAccountOpen(false)} className="flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-stone-700 hover:bg-stone-50 hover:text-stone-950 rounded-lg transition-all">
                                                <MapPin size={16} /> Adrese
                                            </Link>
                                            <div className="h-px bg-stone-100 my-1 mx-2"></div>
                                            <button onClick={() => signOut()} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-50 hover:text-red-700 rounded-lg transition-all">

                        ...
                        {session?.user ? (
                            <div className="relative account-dropdown hidden sm:block z-50">
                                <button
                                    ...
                                    className="flex items-center gap-2 p-1.5 pr-3 bg-white border border-slate-200 rounded-full hover:border-amber-200 transition-colors"
                                >
                                ...
                                <div key={item.label} className="border-b border-slate-50 pb-4 last:border-0">
                                    {item.children ? (
                                        <>
                                            ...
                                            >
                                                {item.label}
                                                <ChevronDown size={20} className={`transition-transform duration-300 ${openMobileSection === item.label ? 'rotate-180 text-amber-500' : ''}`} />
                                            </button>
                                                ...
                                                <div className="flex flex-col gap-2 pl-4 pt-4 border-l-2 border-slate-100 ml-2">
                                                    {item.children.map((child) => (
                                                        <Link
                                                            ...
                                                            href={child.href}
                                                            onClick={() => setMobileMenuOpen(false)}
                                                            className="text-slate-500 hover:text-amber-500 py-2 text-sm font-bold"
                                                        >
                                                            {child.label}
                                                        </Link>
                                                    ))}
                                        ...
                                        <Link
                                            href={item.href}
                                            onClick={() => setMobileMenuOpen(false)}
                                            className={`block text-lg font-black tracking-tight py-2 ${item.highlight ? 'text-amber-500' : 'text-slate-900 hover:text-amber-500'}`}
                                        >
                                            {item.label}
                                        </Link>
                                    )}

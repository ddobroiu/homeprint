...
export default function ConfiguratorPolipropilena({ initialWidth: initW, initialHeight: initH, productImage }: { initialWidth?: number; initialHeight?: number; productImage?: string }) {
    ...
    return (
        <main className="bg-slate-50 min-h-screen py-24 lg:py-32 relative overflow-hidden selection:bg-amber-500 selection:text-white">
            ...
            <div className="container mx-auto px-4 relative z-10">
                          ...
                          <div className="grid grid-cols-4 gap-3 mt-6">
                            {GALLERY.map((src, i) => (
                              <button
                                ...
                                className={`relative rounded-xl aspect-square overflow-hidden border-2 transition-all duration-300 ${activeIndex === i ? "border-amber-500 scale-95" : "border-transparent opacity-40 hover:opacity-100"}`}
                              >
                    ...
                    <div className="relative">
                        <header className="mb-12">
                           <div className="flex items-center gap-3 mb-6">
                              <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse transition-all"></span>
                           ...
                           <h1 className="text-4xl lg:text-6xl font-black text-slate-900 tracking-tighter uppercase italic leading-[0.9] mb-8">Polipropilenă <br/><span className="text-amber-600">Celulară</span></h1>
                           <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 pt-8 border-t border-slate-200">
                              ...
                              <button type="button" onClick={() => setDetailsOpen(true)} className="text-[10px] font-black text-slate-900 uppercase tracking-widest border-b-2 border-amber-600 pb-1 italic hover:text-amber-600 transition-colors">Detalii Tehnice</button>
                           </div>
                        </header>
                        ...
                        <div className="bg-white rounded-[2.5rem] border border-slate-200/60 shadow-xl p-8 sm:p-12 space-y-2 mb-12 relative overflow-hidden">
                           <AccordionStep stepNumber={1} title="Dimensiuni & Volum" summary={summaryStep1} isOpen={activeStep === 1} onClick={() => setActiveStep(1)}>
                              <div className="py-8 space-y-10">
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-3">
                                        ...
                                        <input
                                            ...
                                            placeholder="60"
                                            className="w-full h-16 bg-slate-50 rounded-2xl px-8 font-black text-slate-900 italic text-2xl focus:ring-2 focus:ring-amber-500 outline-none border border-slate-100 transition-all shadow-inner"
                                        />
                                    </div>
                                    <div className="space-y-3">
                                ...
                                {upsellOpportunity && (
                                    <div className="p-8 bg-amber-50 border border-amber-100 rounded-[2rem] flex items-center gap-6 group cursor-pointer hover:bg-amber-100 transition-all shadow-soft" onClick={() => updateInput("quantity", upsellOpportunity.requiredQty)}>
                                        <div className="w-14 h-14 bg-amber-600 rounded-2xl flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform">
                                            <TrendingUp size={24} />
                                        </div>
                                        <div className="flex-1">
                                            <p className="text-[10px] font-black text-amber-700 uppercase tracking-widest italic mb-1">Ofertă Promo</p>
                                        ...
                                        <div className="bg-amber-600 text-white px-4 py-2 rounded-xl text-xs font-black italic shadow-lg">-{upsellOpportunity.discountPercent}%</div>
                                    </div>
                                )}
                              </div>
                           </AccordionStep>
                           ...
                           <AccordionStep stepNumber={3} title="Grafică & Design" summary={summaryStep3} isOpen={activeStep === 3} onClick={() => setActiveStep(3)} isLast={true}>
                              <div className="py-8 space-y-6">
                                ...
                                {input.designOption === 'upload' ? (
                                  <label className="flex flex-col items-center justify-center w-full h-48 bg-slate-50 border-2 border-slate-200 border-dashed rounded-[2rem] cursor-pointer hover:bg-slate-100 hover:border-amber-500/50 transition-all group overflow-hidden">
                                    {uploading ? (
                                      <p className="text-xs font-black text-amber-600 uppercase italic animate-pulse">Se încarcă...</p>
                                    ) : artworkUrl ? (
                                      ...
                                      <>
                                         <UploadCloud className="w-8 h-8 text-slate-300 group-hover:text-amber-500 mb-3" />
                        ...
                        <div className="bg-white p-10 sm:p-12 rounded-[3.5rem] border border-slate-200/60 shadow-elevated relative overflow-hidden group">
                           ...
                           <button onClick={handleAddToCart} className="btn-premium w-full h-24 text-2xl shadow-elevated mb-10 transform active:scale-[0.97] bg-amber-600 text-white">
                           ...
                           <div className="grid grid-cols-2 gap-4">
                              <a href="https://wa.me/40750473111" className="h-14 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center gap-3 text-slate-900 font-black uppercase tracking-widest italic text-[10px] hover:bg-white transition-all shadow-soft">
                                <MessageCircle className="w-4 h-4 text-amber-500" /> WhatsApp
                              </a>
                              <Link href="/contact" className="h-14 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center gap-3 text-slate-900 font-black uppercase tracking-widest italic text-[10px] hover:bg-white transition-all shadow-soft">
                                <Info className="w-4 h-4 text-amber-500" /> Cerere Ofertă
                              </Link>
                           </div>
                        </div>
                    </div>
                ...
                <div className="mt-24 lg:mt-32 max-w-[1400px] mx-auto">
                   <div className="flex gap-4 p-2 bg-white border border-slate-200/60 rounded-3xl w-fit mb-12 shadow-soft overflow-x-auto no-scrollbar">
                      <button onClick={() => setActiveProductTab("descriere")} className={`px-10 py-5 rounded-2xl text-[10px] font-black uppercase tracking-widest italic transition-all whitespace-nowrap ${activeProductTab === "descriere" ? "bg-amber-600 text-white shadow-lg" : "text-slate-400 hover:text-slate-900"}`}>Soluții Economice</button>
                      <button onClick={() => setActiveProductTab("faq")} className={`px-10 py-5 rounded-2xl text-[10px] font-black uppercase tracking-widest italic transition-all whitespace-nowrap ${activeProductTab === "faq" ? "bg-amber-600 text-white shadow-lg" : "text-slate-400 hover:text-slate-900"}`}>Întrebări</button>
                   </div>
                   ...
                   <div className="bg-white p-12 lg:p-24 rounded-[4rem] border border-slate-200/60 shadow-premium min-h-[500px]">
                      {activeProductTab === 'descriere' ? (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                          <div>
                            <h2 className="text-4xl lg:text-5xl font-black text-slate-900 mb-8 uppercase italic tracking-tighter">Eficiență <br/><span className="text-amber-600">Alveolară</span></h2>
                          ...
                          <div className="relative aspect-[4/5] rounded-[3rem] overflow-hidden border-8 border-white shadow-2xl bg-slate-100">
                            ...
                            <div className="absolute inset-x-8 bottom-8 top-1/2 bg-white/70 backdrop-blur-xl rounded-[3rem] p-12 border border-white flex flex-col justify-end text-center">
                               <TrendingUp className="text-amber-600 mb-6 mx-auto" size={40} />
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 backdrop-blur-xl p-6" onClick={() => setDetailsOpen(false)}>
                    <div className="relative w-full max-w-2xl bg-white rounded-[3rem] p-12 lg:p-16 border-white shadow-3xl animate-in fade-in zoom-in duration-300" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="space-y-8 text-slate-600 font-medium italic leading-relaxed">
                            <div>
                                <h4 className="text-amber-600 font-black uppercase text-[10px] tracking-widest mb-2">Orientarea Nervurilor</h4>
                            ...
                            <div>
                                <h4 className="text-amber-600 font-black uppercase text-[10px] tracking-widest mb-2">Imprimare UV Directă</h4>
                            ...
                            <div>
                                <h4 className="text-amber-600 font-black uppercase text-[10px] tracking-widest mb-2">Fixare cu Ocheți</h4>

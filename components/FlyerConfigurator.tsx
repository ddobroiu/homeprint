      ...
      <div className="container mx-auto px-4 py-10 lg:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div className="p-2 grid grid-cols-4 gap-2">
                {GALLERY.map((src, i) => (
                  <button 
                    ...
                    className={`relative rounded-lg aspect-square overflow-hidden ${activeIndex === i ? "ring-2 ring-offset-2 ring-amber-500" : "hover:opacity-80"}`}
                  >
                    <Image 
                      src={src} 
                      alt="Miniatura" 
          ...
          <div>
            <header className="mb-6">
              ...
              <div className="mb-4 p-3 bg-gradient-to-r from-green-50 to-amber-50 border border-green-200 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-2">
                      {FLYER_CONSTANTS.SIZES.map((s: any) => <button key={s.key} onClick={() => setSizeKey(s.key)} className={`p-3 rounded-lg border-2 text-sm ${sizeKey === s.key ? 'border-amber-600 bg-amber-50' : 'border-gray-300 bg-white hover:border-gray-400'}`}>{s.label}</button>)}
                    </div>
                  </div>
                  <div>
                    <label className="field-label">Hârtie</label>
                    <div className="grid grid-cols-1 gap-2 mt-2">
                      {FLYER_CONSTANTS.PAPER_WEIGHTS.map((p: any) => <button key={p.key} onClick={() => setPaperWeightKey(p.key)} className={`p-3 rounded-lg border-2 text-sm ${paperWeightKey === p.key ? 'border-amber-600 bg-amber-50' : 'border-gray-300 bg-white hover:border-gray-400'}`}>{p.label}</button>)}
                    </div>
                  </div>
                </div>
              </AccordionStep>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2">
                      <button onClick={() => setTwoSided(false)} className={`p-3 rounded-lg border-2 text-sm ${!twoSided ? 'border-amber-600 bg-amber-50' : 'border-gray-300 bg-white hover:border-gray-400'}`}>Față</button>
                      <button onClick={() => setTwoSided(true)} className={`p-3 rounded-lg border-2 text-sm ${twoSided ? 'border-amber-600 bg-amber-50' : 'border-gray-300 bg-white hover:border-gray-400'}`}>Față-verso</button>
                    </div>
                  </div>
                </div>
        ...
        <div className="mt-8 lg:mt-12 bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800">
          <nav className="border-b border-gray-200 dark:border-slate-800 flex">
            <button onClick={() => setActiveProductTab('descriere')} className={`px-4 py-3 font-medium text-sm transition-colors border-b-2 ${activeProductTab === 'descriere' ? 'border-amber-600 text-amber-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-300'}`}>Descriere</button>
            <button onClick={() => setActiveProductTab('recenzii')} className={`px-4 py-3 font-medium text-sm transition-colors border-b-2 ${activeProductTab === 'recenzii' ? 'border-amber-600 text-amber-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-300'}`}>Recenzii</button>
            <button onClick={() => setActiveProductTab('faq')} className={`px-4 py-3 font-medium text-sm transition-colors border-b-2 ${activeProductTab === 'faq' ? 'border-amber-600 text-amber-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-300'}`}>FAQ</button>
          </nav>

...
export default function TrackOrderPage() {
    ...
    const getStatusInfo = (status: string) => {
        switch (status) {
            case "active": return { label: "În Producție", color: "text-amber-600", bg: "bg-amber-50", icon: Clock, step: 1 };
            case "fulfilled": return { label: "Expediată", color: "text-amber-600", bg: "bg-amber-50", icon: Truck, step: 3 };
    ...
    return (
        <main className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 flex flex-col items-center pt-28">
            ...
            <div className="mb-8">
                <Link href="/">
                    <span className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-600">HomePrint</span>
                </Link>
            </div>

            {/* HEADER */}
            <div className="text-center mb-10 max-w-xl">
                <div className="inline-flex items-center justify-center p-3 bg-white rounded-2xl shadow-sm mb-4 text-amber-600">
                    <Search size={32} />
                </div>
                    ...
                    <form onSubmit={handleTrack} className="space-y-5">
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-8 pr-4 py-3 rounded-xl border border-slate-200 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 outline-none font-medium text-slate-900 placeholder:text-slate-500 transition-all"
                                    placeholder="Ex: 1024"
                                    required
                                />
                            </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 outline-none font-medium text-slate-900 placeholder:text-slate-500 transition-all"
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-amber-200 transition-all hover:scale-[1.02] disabled:opacity-70 disabled:hover:scale-100 flex items-center justify-center gap-2"
                        >
                            {loading ? (
                        ...
                        <div className="p-6 sm:p-8">
                            ...
                            <div className="mb-10">
                                ...
                                <div className="relative">
                                    ...
                                    <div
                                        className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-amber-600 rounded-full -z-10 transition-all duration-1000"
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <div className="flex flex-col items-center gap-2">
                                            <div className="w-8 h-8 rounded-full bg-amber-600 flex items-center justify-center text-white shadow-lg shadow-amber-200">
                                                <CheckCircle size={16} />
                                            </div>
                                        ...
                                        {/* Step 2: Producție */}
                                        <div className="flex flex-col items-center gap-2">
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shadow-lg transition-colors ${getStatusInfo(order.status).step >= 1 ? 'bg-amber-600 text-white shadow-amber-200' : 'bg-white border-2 border-slate-200 text-slate-300'}`}>
                                                <Package size={16} />
                                            </div>
                                        ...
                                        {/* Step 3: Expediat */}
                                        <div className="flex flex-col items-center gap-2">
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shadow-lg transition-colors ${getStatusInfo(order.status).step >= 3 ? 'bg-amber-500 text-white shadow-amber-200' : 'bg-white border-2 border-slate-200 text-slate-300'}`}>
                                                <Truck size={16} />
                                            </div>
                                            <span className={`text-xs font-bold ${getStatusInfo(order.status).step >= 3 ? 'text-amber-600' : 'text-slate-500'}`}>Expediată</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {/* AWB Info */}
                                {order.awb && (
                                    <div className="mt-8 p-4 bg-amber-50 border border-amber-100 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                        <div className="flex items-center gap-3">
                                            <div className="p-2 bg-white rounded-lg text-amber-600 shadow-sm"><Truck size={20} /></div>
                                            <div>

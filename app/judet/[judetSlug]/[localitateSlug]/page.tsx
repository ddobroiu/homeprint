...
export async function generateMetadata({ params }: Params) {
    ...
    const description = `Fototapet, tablouri canvas, postere și autocolante decorative, cu livrare în ${loc.name}, jud. ${judet.name}. Preț calculat pe loc, producție în 1-3 zile, plată la livrare.`;
                ...
                <div className="container mx-auto grid gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[1.4fr_1fr] lg:items-center lg:py-14">
                    <div>
                        <nav aria-label="Breadcrumb" className="mb-4 flex flex-wrap items-center gap-1.5 text-xs text-slate-500">
                            ...
                            <Link href={`/judet/${judet.slug}`} className="hover:text-amber-700">{judet.name}</Link>
                            <span aria-hidden>/</span>

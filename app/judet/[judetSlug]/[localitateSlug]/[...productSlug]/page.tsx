    ...
    return (
        <div className="bg-[#fafafc] min-h-screen font-sans overflow-x-hidden w-full max-w-full box-border">
                        ...
                        <div className="flex-1 w-full order-2 lg:order-1 lg:pt-8">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 mb-12">
                                ...
                                <a 
                                    ...
                                    className="px-10 py-5 bg-white text-slate-900 border-2 border-slate-200 rounded-2xl font-black text-lg hover:border-amber-500 hover:text-amber-600 transition-all flex items-center justify-center gap-3 shadow-sm active:scale-95 group uppercase italic tracking-tighter"
                                >
            ...
            <div className="bg-slate-950 py-32 relative overflow-hidden">
                ...
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1/2 h-64 bg-amber-500/10 blur-[120px] rounded-full" />
                
                <div className="max-w-7xl mx-auto px-4 relative z-10">
                    <div className="text-center mb-20">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-amber-500/10 rounded-full text-amber-400 font-black text-[10px] uppercase tracking-[0.2em] mb-6 border border-amber-500/20">
                            <ShieldCheck size={14} /> Certificare Calitate HomePrint
                        </div>
                        <h2 className="text-4xl md:text-7xl font-black text-white mb-6 tracking-tighter uppercase leading-[0.9] max-w-4xl mx-auto">
                            Standardul Premium în <span className="text-amber-400">{loc.name}</span>
                        </h2>
                        <p className="text-slate-400 max-w-2xl mx-auto font-medium text-lg leading-relaxed">
                            Nu facem compromisuri. Fiecare comandă de {product.title.toLowerCase()} expediată în județul {judet.name} trece printr-un control riguros sub semnătura HomePrint.
                        </p>
                    </div>
    
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {[
                            { title: "Culori Vibrante", desc: "Folosim echipamente de ultimă generație cu cerneluri rezistente la raze UV și intemperii, garantate pentru ani de zile.", icon: <Sparkles size={28} />, color: "amber" },
                            ...
                            <div key={i} className="group relative">
                                <div className="absolute inset-0 bg-amber-500/5 blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
                                <div className="relative h-full bg-white/5 backdrop-blur-sm p-10 rounded-[2.5rem] border border-white/10 hover:border-amber-500/30 transition-all duration-500 flex flex-col">
                                    <div className={`w-16 h-16 rounded-2xl bg-slate-900 border border-white/10 flex items-center justify-center mb-8 text-amber-400 group-hover:scale-110 group-hover:bg-amber-500 group-hover:text-white transition-all duration-500`}>
                                        {f.icon}
                                    </div>
                                    <h3 className="text-2xl font-black text-white mb-4 uppercase tracking-tight group-hover:text-amber-400 transition-colors">{f.title}</h3>
                                    ...
                                    <div className="mt-6 pt-6 border-t border-white/5 flex items-center justify-between opacity-50 group-hover:opacity-100 transition-opacity">
                                        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-[0.2em]">Verificat HomePrint</span>
                                        <CheckCircle2 size={14} className="text-amber-500/40 group-hover:text-amber-400 transition-colors" />
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            <div className="bg-white py-24 border-t border-slate-100">
                <div className="max-w-4xl mx-auto px-4">
                    <div className="text-center mb-16">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-50 border border-slate-100 text-slate-500 text-[10px] font-black uppercase tracking-widest mb-4">
                            <HelpCircle size={14} className="text-amber-500" /> SUPORT LOCAL {loc.name.toUpperCase()}
                        </div>

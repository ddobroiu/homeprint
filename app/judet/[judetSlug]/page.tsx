            ...
            <section className="border-b border-slate-100 bg-gradient-to-b from-amber-50/60 to-white">
                <div className="container mx-auto px-4 py-8 sm:px-6 lg:py-14">
                    <nav aria-label="Breadcrumb" className="mb-4 flex items-center gap-1.5 text-xs text-slate-500">
                        <Link href="/judet" className="hover:text-amber-700">Județe</Link>
                    ...
                    <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 sm:text-5xl">
                        Decor printat în județul <span className="text-amber-600">{judet.name}</span>
                    </h1>
                    <p className="mt-4 max-w-2xl text-base text-slate-600 sm:text-lg">
                        Fototapet, tablouri canvas, postere și autocolante decorative, livrate la adresă oriunde în județ.
                        {profile && ` Zonă cunoscută pentru ${profile.industrii.slice(0, 2).join(" și ")}; ${profile.reper} e unul dintre reperele ei.`}
                    </p>
                    <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                        <a href="#localitati" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-amber-600 px-7 py-4 text-base font-bold text-white shadow-lg shadow-amber-600/25 transition hover:bg-amber-500">
                        ...
                        <div className="mt-6 flex flex-wrap gap-2">
                            {mainTowns.map((l) => (
                                <Link key={l.slug} href={`/judet/${judet.slug}/${l.slug}`}
                                    className="inline-flex items-center gap-1.5 rounded-xl border border-amber-200 bg-white px-4 py-2.5 font-semibold text-slate-900 hover:border-amber-400 hover:text-amber-700">
                                    <MapPin size={15} className="text-amber-600" /> {l.name}
                                </Link>
                            ))}
                        </div>
                    )}

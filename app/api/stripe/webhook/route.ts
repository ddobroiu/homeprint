...
export async function POST(req: NextRequest) {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        // FILTRARE DUPĂ SURSĂ
        const allowedSources = ['homeprint.ro', 'visionboard.ro', 'prynt.ro', 'euprint.ro', 'tablou.net', 'shopprint.ro'];
        const source = String(session.metadata?.source || 'homeprint.ro').toLowerCase();

        if (!allowedSources.includes(source)) {
            console.log(`[HomePrint Webhook] Ignored event for source: ${source}`);

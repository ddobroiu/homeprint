 ...
 *
 * Acoperă toate liniile din `getProducts()` (bannere, canvas, semnalistică,
 * fonduri EU, configuratoare, campanii SEO + masă).
 */
export function googleProductCategoryId(product: Product): string {
  ...
  // --- Rute fixe (pagini produs / configurator) — prioritate maximă ---
  ...
  // --- metadata.category (agregare din lib/products.ts) ---

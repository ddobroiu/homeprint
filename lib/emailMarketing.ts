...
export function generateNewsletterCampaign(theme: 'weekly' | 'promotional' | 'educational') {
  ...
  switch (theme) {
    case 'weekly':
      return {
        subject: 'Săptămâna aceasta la HomePrint: Noutăți și Oferte',
...
export async function sendConfiguratorWelcomeEmail(subscription: NewsletterSubscription) {
  ...
  const html = getHtmlTemplate({
    ...
    footerText: "Mulțumim că te-ai alăturat comunității HomePrint!"
  });
  ...
  await sendEmail({
    from: 'HomePrint <contact@HomePrint.ro>',
...
export async function sendPostPurchaseFollowUp(email: string, name: string, orderId: string, orderNo: number) {
  ...
  const html = getHtmlTemplate({
    ...
    footerText: "Echipa HomePrint vă mulțumește!"
  });
  ...
  try {
    await sendEmail({
      from: 'HomePrint <contact@HomePrint.ro>',
      to: email,
      subject: `🎁 Avem un cadou pentru tine (Comanda ta #${orderNo} la HomePrint)`,
      html: enhancedHtml,
    });
    return true;
  } catch (error) {

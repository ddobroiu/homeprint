...
export function getConfigForSource(source?: string | null) {
  ...
  if (s.includes('homeprint.ro')) {
    return { name: 'HomePrint.ro', url: 'www.homeprint.ro', email: 'contact@homeprint.ro' };
  }
...
export async function sendNewOrderAdminEmail(order: any, customContent?: string) {
  try {
    ...
    const html = getPremiumHtmlTemplate({
      ...
      buttonUrl: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://www.HomePrint.ro'}/admin/orders/${order.id}`,
      brandConfig: config
    });
